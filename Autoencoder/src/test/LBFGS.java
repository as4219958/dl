...
public class LBFGS {
	...
	public static void lbfgs(int n, int m, double[] x, double f, double[] g,
		...
		if (iflag[0] == 0) {
			...
			/*
			  W=[f'_k,RHO,ALPHA,S,Y]
			      其中保证第一次计算得到的值放在第一个，后面的依次循环存放
			      比如在RHO_1在第二次迭代中计算得到，那存放RHO_i的地址为(k-2)%m+1
			      而y_1在第二次迭代中计算得到，那么存放Y_i的地址为IYPT+(k-2)%m+1
			 */
			...
			gnorm = Math.sqrt(ddot(n, g, 0, 1, g, 0, 1));//||g(0)||
			stp1 = 1 / gnorm;//初始化第一次试探步长
			ftol = 0.0001;
			maxfev = 20;
			if (iprint[1 - 1] >= 0)
	...
	public static void lb1(int[] iprint, int iter, int nfun, double gnorm,
		...
		if (iter == 0) {
			...
			System.err
					.println("*************************************************");
			System.err.println("titnfntfunctgnormtsteplength");
		} else {
			...
			if (iprint[1 - 1] != 0) {
				if ((iter - 1) % iprint[1 - 1] == 0 || finish) {
					if (iprint[2 - 1] > 1 && iter > 1)
						System.err.println("titnfntfunctgnormtsteplength");
					System.err.println("t" + iter + "t" + nfun + "t" + f + "t"
							+ gnorm + "t" + stp[0]);
				} else {
					return;
				}
